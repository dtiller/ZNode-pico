...
// TODO Implement
    result = 0; // write( serial->fd, buffer, length);
...
// TODO Implement
    result = 0; // read( serial->fd, buffer, bufsize);
    if (result == -1)
    {
        if (errno == EAGAIN)
        {
...
int xbee_ser_open( xbee_serial_t *serial, uint32_t baudrate)
{
    ...
    if (serial->fd <= 0) {
// TODO Implement
        serial->fd = 1; // open( serial->device, O_RDWR | O_NOCTTY | O_NONBLOCK);
        if (serial->fd < 0)
        {
            #ifdef XBEE_SERIAL_VERBOSE
...
// TODO Implement
    if (0) // (ioctl( serial->fd, enabled ? TIOCSBRK : TIOCCBRK) == -1)
    {
        #ifdef XBEE_SERIAL_VERBOSE
...
// TODO Implement
    if (0) // (tcgetattr( serial->fd, &options) == -1)
    {
        #ifdef XBEE_SERIAL_VERBOSE
...
// TODO Implement
    if (0) // (tcsetattr( serial->fd, TCSANOW, &options) == -1)
    {
        #ifdef XBEE_SERIAL_VERBOSE
